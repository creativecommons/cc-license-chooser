...
def key2canonical(key):
    ...
    if key not in pofile.strings:
        ...
        print("had to strip key, hope it helped! {}".format(key))
...
def pofd2converted(pofd):  # noqa E302: ignoring because of docstring 
    ...
    if value:  # only bother converting those that are translated
        try:
            ...
            print("unfortunately, could not find English for {} so adding"
                  "it with old key".format(key))
...
def country_id2name(country_id, language):
    ...
    country_key = "country.{}".format(country_id)
...
def extremely_slow_translation_function(s, out_lang):
    ...
    for entry in en_po._messages:
        if en_po[entry].string == u:
            found_key = entry
            print >> debug_stream, "yahoo, found {}".format(found_key)
